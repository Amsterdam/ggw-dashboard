 ...
 * @returns {number}
 */
export function decodeString(s: string, len = s.length): number {
  if (len > 1) {
    const quotient = s.substr(0, len - 1);
    const remainder = s.charAt(len - 1);
    return 62 * decodeString(quotient, len - 1) + decodeString(remainder, 1);
  } else {
 ...
 * @returns {number}
 */
export function base62DecodeAngle(s: string, precision: number): number {
  if (s[0] === "-") {
